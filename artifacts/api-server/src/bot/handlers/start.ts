...
export async function handleStart(bot: TelegramBot, msg: TelegramBot.Message): Promise<void> {
  ...
  try {
    ...
    if (isNew) {
      ...
      welcomeText = `🎰 *Welcome to RAIZO GAMES, ${user.first_name || "Player"}!*\n\n`;
      welcomeText += `🔥 *PvP Casino Bot* — Fast. Secure. Competitive.\n\n`;
      welcomeText += `🎁 *Newbie Bonus:* ${formatUSD(bonus)} USDT added to your account!\n`;
      welcomeText += `_(Bonus expires in 7 days. 7x wager required to withdraw winnings.)_\n\n`;
      welcomeText += `━━━━━━━━━━━━━━━━━━━\n`;
      welcomeText += `💰 *Deposit & Play:*\n`;
      welcomeText += `• Min bet: $0.02 USDT\n`;
      welcomeText += `• Min deposit: $1 USDT\n`;
      welcomeText += `• Instant withdrawals (min $0.50)\n\n`;
      welcomeText += `🎮 *Games:* Dice 🎲 | Slots 🎰 | Basketball 🏀\n`;
      welcomeText += `Bowling 🎳 | Darts 🎯 | Football ⚽ | RPS ✊\n\n`;
      welcomeText += `Use /referral to invite friends and earn 5% commission!`;
    } else {
      welcomeText = `🎰 *Welcome back, ${user.first_name || "Player"}!*\n\n`;
      welcomeText += `💵 *Balance:* ${formatUSD(parseFloat(String(dbUser.real_balance)))}\n`;
      if (parseFloat(String(dbUser.bonus_balance)) > 0) {
        welcomeText += `🎁 *Bonus:* ${formatUSD(parseFloat(String(dbUser.bonus_balance)))}\n`;
      }
